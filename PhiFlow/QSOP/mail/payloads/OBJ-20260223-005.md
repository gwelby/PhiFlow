...
`let lambda = coherence_formula(depth)` inside an intention block assigns `2.0` (the argument value) not `0.618` (the function return value). The `breathe()` function works inside intention blocks — so simple function calls work. The failure is specific to functions that call other functions (nested call depth > 1): `coherence_formula` calls `phi_power`, and that's where the chain breaks.
...
**Risk 3: Parser-level vs evaluator-level — wrong layer diagnosis**
...
1. Failing test passes: `test_nested_function_call_in_intention_block_returns_correct_value` → resonated value within 0.001 of 0.618
2. `claude.phi` resonates correctly: `cargo run --bin phic -- examples/claude.phi` produces `🔔 Resonating Field: 0.6180Hz`
3. Regression clean: All existing tests pass — 13 evaluator tests, 1 integration test, 3 stream tests. Zero regressions.
...
```markdown
## [Codex] OBJ-20260223-005 — Bugfix: Nested Function Calls in Intention Blocks
**Date:** 2026-02-23
