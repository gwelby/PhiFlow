...
**Lane:** C (QDrive Codex — defensive diagnostics)
...
| `E003_EXPECTED_TOKEN` | `expect()` call fails — parser wanted a specific token | `function f(x Number) -> Number { return x }` |
...
```
"Unexpected character" at line X, column Y  → E004_UNEXPECTED_CHAR, extract line/col
"Unexpected end of file"                    → E002_UNEXPECTED_EOF, line=0, col=0
"Expected X, found Y"                       → E003_EXPECTED_TOKEN, extract X and Y
"Unexpected token in statement: Eof"        → E002_UNEXPECTED_EOF (Eof = unclosed)
"Unexpected token"                          → E001_UNEXPECTED_TOKEN, extract token name
(any other)                                 → E001_UNEXPECTED_TOKEN, found=err, line=0
```
...
| File | Action |
|------|--------|
| `src/phi_diagnostics.rs` | CREATE — `PhiDiagnostic` struct, `Display` impl |
| `src/lib.rs` | ADD — `pub mod phi_diagnostics; pub use phi_diagnostics::PhiDiagnostic;` |
| `src/parser/mod.rs` | ADD — `parse_phi_program_with_diagnostics()` + `string_to_diagnostic()` |
| `src/main_cli.rs` | ADD — `--json-errors` flag, use `parse_phi_program_with_diagnostics` |
| `tests/phi_diagnostics_tests.rs` | CREATE — 5 fixture tests |
| `docs/ERROR_RECOVERY.md` | CREATE — 5 sections, one per error code |
...
## 10b. QDrive Contract Amendments (v1 locks — required for Lane D frictionlessness)

QDrive Codex reviewed this spec and locked three additional contracts. These are binding:

**Contract 1 — JSON output purity**
...
**Contract 2 — Exit code stability**
```
...
**Contract 3 — JSON schema freeze (v1)**
...
1. All 5 fixture tests fail before fix, pass after fix — document both runs in ACK.
2. `parse_phi_program` signature unchanged — all existing tests pass.
3. `cargo run --bin phic -- examples/claude.phi` still outputs `🔔 Resonating Field: 0.6180Hz` (zero regression on happy path).
...
7. Full regression: all existing tests pass — evaluator, integration, stream, conformance, IR.
