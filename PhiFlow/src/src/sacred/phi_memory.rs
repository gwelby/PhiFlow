...
mod tests {
    ...
    fn test_phi_optimal_sizing() {
        ...
        // Test that PHI scaling produces expected ratios
        let size_100 = allocator.calculate_phi_optimal_size(100);
        let size_1000 = allocator.calculate_phi_optimal_size(1000);
        
        assert!(size_100 > 100);
        assert!(size_1000 > 1000);
        
        // Verify golden ratio relationship
        let ratio = size_1000 as f64 / size_100 as f64;
        assert!((ratio - PHI).abs() < 0.1); // Allow for power-of-2 rounding
    }
