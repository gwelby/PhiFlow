                ...
                match &inst.node {
                    ...
                    PhiIRNode::Resonate { value, direction, .. } => {
                        ...
                        if self.collapsed_qubits.contains(&target_q) {
                            self.source.push_str(&format!("    // WARNING: Gating post-collapsed qubit q[{target_q}]\n"));
                        }
                    ...
                    PhiIRNode::CoherenceCheck => {
                        let target_q = self.current_qubit_idx()?;
                        if self.collapsed_qubits.contains(&target_q) {
                            self.source.push_str(&format!("    // WARNING: Coherence post-collapsed qubit q[{target_q}]\n"));
                        }
...
mod tests {
    ...
    fn test_full_pipeline_resonate_value() {
        ...
        assert!(
            code.contains("rz(0.72 * pi + pi) q[0];"),
            "Expected rz(0.72 * pi + pi) but got something else.\nIR dump:\n{}\nQASM:\n{}",
            ir_dump,
            code
        );
    }
    ...
    fn test_openqasm_emission_basic() {
        ...
        assert!(code.contains("rz(0.6180339887 * pi + pi) q[0];")); // coherence on Eagles (q[0])
        assert!(code.contains("rz(pi/2 + pi) q[1];")); // resonate on Chiefs (q[1])
    ...
    fn test_openqasm_hardware_stress_injection() {
        ...
        assert!(code_stress.contains("rz(0.2 * pi) q[0];"));
        assert!(code_stress.contains("rz(-pi/2) q[0];"));
    }
    ...
    fn test_openqasm_resonate_confidence_values() {
        ...
        assert!(code.contains("rz(0.72 * pi + pi) q[0];"));
        assert!(!code.contains("rz(pi/2 + pi) q[0];"));
    }
    ...
    fn test_openqasm_team_direction() {
        ...
        assert!(code.contains("rz(0.72 * pi + pi) q[0];"));
        assert!(code.contains("rz(0.28 * pi + pi) q[1];"));
    }
    ...
    fn test_openqasm_witness_mid_circuit() {
        ...
        let coherence_idx = code
            .find("rz(0.6180339887 * pi + pi) q[1];")
