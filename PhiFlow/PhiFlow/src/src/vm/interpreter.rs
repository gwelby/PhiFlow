...
use crate::compiler::ast::{
    ...
    ConsciousnessCondition, ComparisonOperator
};
...
impl PhiFlowInterpreter {
    ...
    pub async fn evaluate_expression(&mut self, expr: &PhiFlowExpression) -> RuntimeResult<PhiFlowValue> {
        ...
        while let Some(frame) = stack.pop() {
            match frame {
                EvaluationFrame::Expression(expr) => {
                    match expr {
                        ...
                        PhiFlowExpression::Let { variable, value, .. } => {
                            // For Let expressions, we need to evaluate the value first
                            // This is a simple synchronous operation for now
                            let val = self.evaluate_expression_simple(&value)?;
                        ...
                        PhiFlowExpression::ArrayIndex { array, index } => {
                            ...
                            let array_val = self.evaluate_expression_simple(&array)?;
                            let index_val = self.evaluate_expression_simple(&index)?;
                        ...
                        PhiFlowExpression::FunctionCall { name, args } => {
                            // Handle function calls with stack frames to avoid async recursion
                            if args.is_empty() {
                                // No arguments, call the function directly
                                let result = self.call_function_with_args(&name, &[])?;
                                result_stack.push(result);
                            } else {
                                // Push function call frame and evaluate first argument
                ...
                EvaluationFrame::WhileLoop { condition, body, iterations } => {
                    ...
                    if iterations == 0 {
                        ...
                        // We just finished executing the body, check condition again
                        let body_result = result_stack.pop().unwrap_or(PhiFlowValue::Nil);
                        
                        // Check the condition again to see if we should continue
                        // For now, we need to evaluate the condition synchronously
                        let condition_value = self.evaluate_expression_simple(&condition)?;
                        
                        if self.is_truthy(&condition_value) {
                            // Continue loop - execute body again
                            stack.push(EvaluationFrame::WhileLoop {
                                condition: condition.clone(),
                                body: body.clone(),
                                iterations: iterations + 1,
                            ...
                            // Condition false, exit with the last body result
                            result_stack.push(body_result);
                        }
                    }
                }
                EvaluationFrame::FunctionCall { name, args, mut evaluated_args, current_arg } => {
                    if current_arg < args.len() {
                        // Get the result of the current argument
                        ...
                        if current_arg + 1 < args.len() {
                            ...
                            let result = self.call_function_with_args(&name, &evaluated_args)?;
                            result_stack.push(result);
                        }
                    }
                }
    ...
    // Synchronous function call evaluation to avoid async recursion
    fn call_function_with_args(&mut self, name: &str, args: &[PhiFlowValue]) -> RuntimeResult<PhiFlowValue> {
        // Built-in functions
        match name {
            "print" => {
                for arg in args {
            ...
            "len" => {
                ...
                match &args[0] {
                    PhiFlowValue::Array(arr) => Ok(PhiFlowValue::Number(arr.len() as f64)),
                    PhiFlowValue::String(s) => Ok(PhiFlowValue::Number(s.len() as f64)),
            ...
            "push" => {
                ...
                match &args[0] {
                    PhiFlowValue::Array(arr) => {
                        ...
                        Ok(PhiFlowValue::Array(new_arr))
                    }
                    _ => Err(RuntimeError::TypeError {
                        expected: "Array".to_string(),
                        found: "non-array type".to_string(),
                    }),
                }
            }
            "pop" => {
            ...
            _ => {
                // User-defined function - this is simplified for now
                // In a full implementation, we'd need to handle scoping properly
                if let Some(function_expr) = self.functions.get(name).cloned() {
                    if let PhiFlowExpression::FunctionDefinition { parameters, body, .. } = function_expr {
                        ...
                        // Execute function body - simplified synchronous evaluation
                        let result = self.evaluate_expression_simple(&body);
    ...
    // Simple synchronous expression evaluation for basic cases
        ...
        match expr {
            ...
            PhiFlowExpression::Array(elements) => {
                ...
                for element in elements {
                    values.push(self.evaluate_expression_simple(element)?);
                }
                Ok(PhiFlowValue::Array(values))
            }
            PhiFlowExpression::ArrayIndex { array, index } => {
                let array_val = self.evaluate_expression_simple(&array)?;
                let index_val = self.evaluate_expression_simple(&index)?;
            ...
            PhiFlowExpression::BinaryOp { left, operator, right } => {
                let left_val = self.evaluate_expression_simple(left)?;
                let right_val = self.evaluate_expression_simple(right)?;
                self.evaluate_binary_op_values(&left_val, operator, &right_val)
            }
            PhiFlowExpression::UnaryOp { operator, operand } => {
                let operand_val = self.evaluate_expression_simple(operand)?;
                self.evaluate_unary_op_values(operator, &operand_val)
            }
            PhiFlowExpression::Block(expressions) => {
                // Blocks require async evaluation to avoid infinite recursion
                Err(RuntimeError::RuntimeError {
                    message: "Blocks require async evaluation".to_string(),
                })
            }
            _ => Err(RuntimeError::RuntimeError {
                message: "Complex expression requires async evaluation".to_string(),
            })
        }
    }
    ...
    async fn evaluate_function_call(
        &mut self,
        name: &str,
        args: &[PhiFlowExpression],
    ) -> RuntimeResult<PhiFlowValue> {
        // Built-in functions
        match name {
            "print" => {
                for arg in args {
                    let value = self.evaluate_expression(arg).await?;
            ...
            "len" => {
                ...
                let value = self.evaluate_expression(&args[0]).await?;
                match value {
                    PhiFlowValue::Array(arr) => Ok(PhiFlowValue::Number(arr.len() as f64)),
                    PhiFlowValue::String(s) => Ok(PhiFlowValue::Number(s.len() as f64)),
                    _ => Err(RuntimeError::TypeError {
            ...
            "push" => {
                ...
                // Note: This is a simplified push that returns a new array
                // In a full implementation, we'd want mutable arrays
                let array_val = self.evaluate_expression(&args[0]).await?;
                let item_val = self.evaluate_expression(&args[1]).await?;
                
                match array_val {
                    PhiFlowValue::Array(mut arr) => {
                        arr.push(item_val);
                        Ok(PhiFlowValue::Array(arr))
                    }
                    _ => Err(RuntimeError::TypeError {
                        expected: "Array".to_string(),
                        found: "non-array type".to_string(),
            ...
            "phi_spiral" => {
                ...
                let n = self.evaluate_expression(&args[0]).await?;
                if let PhiFlowValue::Number(steps) = n {
                    ...
                    Ok(PhiFlowValue::Array(spiral))
                } else {
                    Err(RuntimeError::TypeError {
                        expected: "number".to_string(),
                        found: "non-number".to_string(),
                    })
                }
            }
            "sacred_resonate" => {
                ...
                let freq = self.evaluate_expression(&args[0]).await?;
                if let PhiFlowValue::SacredFrequency(frequency) = freq {
                    ...
                    Ok(PhiFlowValue::Boolean(true))
                } else {
                    Err(RuntimeError::TypeError {
                        expected: "sacred frequency".to_string(),
                        found: "non-frequency".to_string(),
                    })
                }
            }
            _ => {
                ...
                if let Some(function_expr) = self.functions.get(name).cloned() {
                    if let PhiFlowExpression::FunctionDefinition { parameters, body, .. } = function_expr {
                        ...
                        for (i, param) in parameters.iter().enumerate() {
                            if i < args.len() {
                                let arg_value = self.evaluate_expression(&args[i]).await?;
                                if let Some(old_value) = self.variables.insert(param.name.clone(), arg_value) {
                                    old_vars.insert(param.name.clone(), old_value);
                                }
                            }
                        }
