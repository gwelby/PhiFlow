        ...
        Ok(ast) => {
            ...
            for expr in &ast {
                match expr {
                    ...
                    phiflow::parser::PhiExpression::EmergencyProtocol { .. } => {
                        emergency_count += 1
                    }
                    phiflow::parser::PhiExpression::BiologicalInterface { .. } => {
                        biological_count += 1
                    }
                    phiflow::parser::PhiExpression::FunctionDef { .. } => function_count += 1,
                    phiflow::parser::PhiExpression::ConsciousnessFlow { .. } => {
                        consciousness_flow_count += 1
                    }
                    _ => other_count += 1,
                }
            }
