...
## 10. Demonstration: Autonomous Agent

To demonstrate that the primitives are practical (not merely theoretical), we implemented an autonomous data analysis agent in 80 lines of PhiFlow (`examples/autonomous_agent.phi`). The agent:
...
6. Stops on one of three conditions:
   - **Success:** data confidence ≥ 0.90 (the agent is confident in its analysis)
   - **Emergency:** coherence ≤ 0.10 (the agent's self-alignment has degraded)
   - **Timeout:** cycle ≥ 15 (hard safety brake)

The key distinction the agent makes is between **confidence** (assessment of the data, external) and **coherence** (assessment of itself, internal). These are two different stopping conditions for two different reasons. The emergency stop — where the agent detects its own coherence has dropped and aborts without external intervention — is the primitive that matters most for autonomous AI safety.

When run, the agent processes 10 cycles, resonates 6 significant findings (confidence values from 0.833 to 0.909), and stops via the success path when confidence reaches 0.909. The witness log contains 12 checkpoint events. The resonance field contains 7 values (6 findings + 1 final coherence). This audit trail is generated by the language primitives as a side effect of normal execution — no explicit logging code is needed.

The demo runs in the tree-walking evaluator, the browser demo (via WebAssembly), and is described in detail in `docs/autonomous-agent-demo.md`.
