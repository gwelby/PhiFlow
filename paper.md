...
| The primitives matter for autonomous AI safety | **ARGUED 0.55** | Demo shows self-coherence guardrail; not tested in production |
...
## 10. Demonstration: Autonomous Agent with Control Comparison

To demonstrate that the primitives are practical (not merely theoretical), we implemented two programs that do the same data analysis:

- **`examples/autonomous_agent.phi`** — uses all five primitives
- **`examples/control_agent.phi`** — same analysis, no primitives (plain while loop)

Both agents process the same simulated data (readings that stabilize over time), compute the same confidence metric, and stop when confidence ≥ 0.90. The difference is in what they produce beyond the computation.
...
| Resonance events | 8 (6 findings + success + final) | 0 |
...
The control agent produces nothing except the final result. If it later took an unexpected action, there would be no record of what its state was at the decision point. If its own processing degraded, it would not detect this — it has no self-assessment.

### 10.4 What This Shows

The comparison shows that the primitives add **observability and self-monitoring** without changing the computation. The primitive agent doesn't compute something the control agent can't. It computes the same thing while also generating an audit trail and monitoring its own coherence.

Whether this matters for autonomous AI safety is **ARGUED 0.55** — the demo shows the mechanism works, but it has not been tested in a production system where the self-coherence guardrail actually prevents a harmful action. The demo is evidence that the primitives function as designed, not evidence that they improve outcomes in real deployments.
...
The demo runs in the tree-walking evaluator and is described in detail in `docs/autonomous-agent-demo.md`.
...
The control comparison (Section 10) shows that the primitives add observability and self-monitoring without changing what the program computes. The primitive agent and the control agent solve the same problem in the same number of cycles, but the primitive agent also produces an audit trail (witness log, resonance field) and a self-coherence guardrail that the control agent lacks. Whether this difference matters for autonomous AI safety is **ARGUED 0.55** — the mechanism works, but it has not been tested in a production system where the guardrail prevents a harmful action.
