...
use crate::compiler::ast::{
    BinaryOperator, BrainwaveType, ComparisonOperator, ConsciousnessCondition, ConsciousnessMetric,
    LogicalOperator, PhiFlowExpression, PhiFlowProgram, PhiFlowType, QuantumGate, QuantumGateType,
    UnaryOperator,
};
...
impl PhiFlowInterpreter {
    ...
    fn init_built_in_functions(&mut self) {
        ...
        self.variables
            .insert("PHI".to_string(), PhiFlowValue::Number(PHI));
        self.variables
            .insert("PI".to_string(), PhiFlowValue::Number(std::f64::consts::PI));
        self.variables
            .insert("E".to_string(), PhiFlowValue::Number(std::f64::consts::E));

        // Add sacred frequencies as constants
        for &freq in SACRED_FREQUENCIES {
            let name = format!("SACRED_{}", freq);
            self.variables
                .insert(name, PhiFlowValue::SacredFrequency(freq));
        }
        ...
        for name in built_ins {
            self.variables.insert(
                name.to_string(),
                PhiFlowValue::BuiltInFunction(name.to_string()),
            );
        }
    }
    ...
    pub async fn evaluate_expression(
        ...
        while let Some(frame) = stack.pop() {
            match frame {
                    ...
                    match expr {
                        ...
                        PhiFlowExpression::FunctionCall { name, args } => {
                            ...
                            let is_built_in = self
                                .variables
                                .get(&name)
                                .map_or(false, |v| matches!(v, PhiFlowValue::BuiltInFunction(_)));
                            let is_user_defined = self.functions.contains_key(&name);

                            if is_built_in || is_user_defined {
                                if args.is_empty() {
                                    if let Some(result) =
                                        self.call_function_with_args(&name, &[], &mut stack)?
                                    {
                ...
                EvaluationFrame::SacredFrequency {
                    frequency,
                    operation,
                    operation_expr: _,
                } => {
                    if operation.is_none() {
                        ...
                        let result =
                            self.evaluate_sacred_frequency_values(frequency, &operation.unwrap())?;
                        result_stack.push(result);
                    }
                }
                EvaluationFrame::Block {
                    expressions,
                    index,
                    last_result,
                } => {
                ...
                EvaluationFrame::ForLoop {
                    ...
                    if iterable_value.is_none() {
                        ...
                        self.variables
                            .insert(variable.clone(), items[current_index].clone());
    ...
    fn evaluate_expression_simple(&self, expr: &PhiFlowExpression) -> RuntimeResult<PhiFlowValue> {
        match expr {
            ...
            PhiFlowExpression::Variable(name) => self
                .variables
                .get(name)
                .cloned()
    ...
    fn evaluate_binary_op_values(
        ...
        match (left_val, right_val) {
            (PhiFlowValue::Number(l), PhiFlowValue::Number(r)) => match operator {
                ...
                BinaryOperator::NotEqual => {
                    Ok(PhiFlowValue::Boolean((l - r).abs() >= f64::EPSILON))
                }
            ...
            (PhiFlowValue::Boolean(l), PhiFlowValue::Boolean(r)) => match operator {
    ...
    async fn evaluate_frequency_lock(
        &mut self,
        target_frequency: u32,
        threshold: f64,
        action: &PhiFlowExpression,
    ) -> RuntimeResult<PhiFlowValue> {
    ...
    async fn evaluate_consciousness_binding(
        ...
        self.variables
            .insert(state_name.to_string(), state_value.clone());
    ...
    async fn evaluate_consciousness_monitor(
        ...
        for metric in metrics {
            let metric_value = match metric {
                ...
                ConsciousnessMetric::EEGChannel(channel) => consciousness
                    .eeg_channels
                    .get(channel)
                    .cloned()
                    .unwrap_or(0.0),
                ConsciousnessMetric::BrainwaveType(wave_type) => consciousness
                    .brainwave_levels
                    .get(wave_type)
                    .cloned()
                    .unwrap_or(0.0),
            ...
            self.variables
                .insert(metric_name, PhiFlowValue::Number(metric_value));
        }
    ...
    fn check_consciousness_condition(
        &self,
        condition: &ConsciousnessCondition,
    ) -> RuntimeResult<bool> {
        let consciousness = self.consciousness.lock().unwrap();
        ...
        match condition {
            ConsciousnessCondition::MetricThreshold {
                metric,
                operator,
                threshold,
            } => {
            ...
            ConsciousnessCondition::FrequencyLock {
                frequency,
                stability_threshold,
            } => {
                if let Some(current_freq) = consciousness.current_frequency {
    ...
    async fn evaluate_quantum_circuit(
        ...
        quantum_backend
            .quantum_states
            .insert(circuit_name, quantum_state.clone());
        ...
        match &gate.gate_type {
            ...
            QuantumGateType::CNOT => {
                if gate.qubits.len() != 2 {
                    return Err(RuntimeError::QuantumError {
                        message: "CNOT gate requires exactly 2 qubits (control, target)"
                            .to_string(),
                    });
                }

                let control_index = qubit_names
                    .iter()
                    .position(|name| name == &gate.qubits[0])
                    .ok_or_else(|| RuntimeError::QuantumError {
        ...
        for i in 0..amplitudes.len() {
            if (i >> qubit_index) & 1 == 0 {
                ...
                amplitudes[i] = num_complex::Complex64::new(
                    norm_factor * (amp_i.re + amp_j.re),
                    norm_factor * (amp_i.im + amp_j.im),
                );
        ...
        match name {
            ...
            _ => {
                ...
                if let Some(function_expr) = self.functions.get(name).cloned() {
                    if let PhiFlowExpression::FunctionDefinition {
                        parameters, body, ..
                    } = function_expr
                    {
    ...
    fn value_to_string(&self, value: &PhiFlowValue) -> String {
        match value {
            ...
            PhiFlowValue::ConsciousnessState {
                ...
                format!(
                    "Consciousness(coherence: {:.3}, clarity: {:.3}, flow: {:.3})",
...
impl Default for PhiFlowInterpreter {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    async fn test_user_function_call() {
        ...
        let result = interpreter
            .evaluate_expression(&function_call)
            .await
            .unwrap();
        assert_eq!(result, PhiFlowValue::Number(5.0));
    }
    ...
    async fn test_witness_intention_resonate() {
        ...
        let expr = PhiFlowExpression::Intention {
            ...
            target: Box::new(PhiFlowExpression::FunctionCall {
                ...
                args: vec![PhiFlowExpression::Witness(Box::new(
                    PhiFlowExpression::Number(528.0),
                ))],
            }),
        };
        ...
        assert!(interpreter
            .observation_history
            .iter()
            .any(|o| o.contains("528")));
        assert!(interpreter.resonance_field.contains_key("healing"));
        assert_eq!(
            interpreter.resonance_field.get("healing").unwrap()[0],
            PhiFlowValue::Number(528.0)
        );
    }
}
