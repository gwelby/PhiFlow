...
fn test_lexer_sacred_frequency() {
    ...
    assert!(tokens.len() >= 2); // Sacred(432), EOF
    if let phiflow::Token::Sacred(freq) = &tokens[0] {
        assert_eq!(*freq, 432);
    } else {
        panic!("Expected Sacred(432) token, got {:?}", tokens[0]);
    }
}
...
#[test]
fn test_lexer_stream_keywords() {
    let source = "stream \"Healing\" { break stream }";
    let mut lexer = PhiFlowLexer::new(source.to_string());
    let tokens = lexer.tokenize().expect("Lexing should succeed");

    // Just verify lexing doesn't panic on keywords
    assert!(tokens.len() >= 6);
}
...
fn test_sacred_frequencies() {
    use phiflow::quantum::types::{
        get_nearest_sacred_frequency, is_sacred_frequency,
    };
...
fn test_quantum_result_structure() {
    ...
    let result = QuantumResult {
        ...
        counts: [("00".to_string(), 512), ("11".to_string(), 512)]
            .iter()
            .cloned()
            .collect(),
...
fn test_phi_harmonic_operation() {
    use phiflow::quantum::types::{PhiAxis, PhiHarmonicOperation};
