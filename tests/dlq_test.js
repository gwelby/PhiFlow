 ...
 * 5. Verifies the message is marked 'timeout' in queue.jsonl
...
const DLQ_PATH = process.env.MCP_DLQ_PATH
    ...
    : path.resolve(__dirname, '../QSOP/mail/dead_letter');
        ...
        try {
            ...
            if (resp.id === 'sweep_req' && resp.result?.content?.[0]?.text) {
                ...
                if (parsed.status === 'success') {
                    ...
                    // 1. Verify queue.jsonl
                    const queue = loadQueueState(QUEUE_PATH, LEGACY_QUEUE_PATH);
                    const msg = queue.find(m => m.id === messageId);
                    if (msg?.status === 'timeout') {
                        console.log(`✅ [DLQ_TEST] queue.jsonl updated to "timeout".`);
                    } else {
                        console.error('❌ [DLQ_TEST] queue.jsonl status is NOT timeout!');
                        process.exit(1);
                    }
