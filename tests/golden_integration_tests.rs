...
";
    ...
    assert!(qasm.contains("ry(pi - (0.72 * pi)) q[1]"), "Expected ry(pi - (0.72 * pi)) for TEAM_B, but got:\n{}", qasm);
}
...
fn test_team_direction_bytecode_roundtrip() {
    ...
    let resonate_opcode_idx = bytes.iter().position(|&b| b == 0x33).expect("OP_RESONATE not found in emitted bytecode");
    assert_eq!(bytes[resonate_opcode_idx + 1], 1, "Direction byte should be 1 for TEAM_B");

    // Verify VM can successfully deserialize the modified bytecode without error
    let _vm = phiflow::phi_ir::vm::PhiVm::from_bytes(&bytes).expect("VM deserialization failed on direction byte");
}
