...
";
    ...
    assert!(
        qasm.contains("ry(0.72 * pi)"),
        "Expected logical ry decomposition for TEAM_A, but got:\n{}",
        qasm
    );
    assert!(
        qasm.contains("ry(pi - (0.72 * pi))"),
        "Expected logical ry decomposition for TEAM_B, but got:\n{}",
        qasm
    );
}
...
";
    ...
    let resonate_idx = qasm.find("ry(0.5 * pi)").expect("resonate (logical) not found");

    assert!(
        measure_idx < resonate_idx,
        "Witness (measure) must appear BEFORE resonate gate in mid-circuit context"
    );
}
...
";
    ...
    assert!(
        ...
        "Incorrect cross-frequency entanglement detected:\n{}",
        qasm
    );
    assert!(
        !qasm.contains("cx q[0], q[2]"),
        "528Hz chain should not entangle to 432Hz qubits:\n{}",
        qasm
    );
}
...
";
    ...
    match res {
        PhiIRValue::Number(n) => assert_eq!(
            n, 1.0,
            "Expected evolution to set result to 1.0 (low coherence path)"
        ),
...
";
    ...
    match res_high {
        PhiIRValue::Number(n) => assert_eq!(
            n, 2.0,
            "Expected evolution to set result to 2.0 (high coherence path)"
        ),
        _ => panic!("Expected number result"),
    }
}
...
#[test]
fn test_team_direction_bytecode_roundtrip() {
    let source = "
    intention \"Test\" {
        resonate 0.72 toward TEAM_B
    }
    ";
    let exprs = phiflow::parser::parse_phi_program(source).expect("parse failed");
    let program = phiflow::phi_ir::lowering::lower_program(&exprs);
    let bytes = phiflow::phi_ir::emitter::emit(&program);

    // OP_RESONATE is 0x33.
    // Format: [RESULT: 0=None] [OP_RESONATE (0x33)] [DIRECTION: 1=TeamB] [VALUE: 1=Some] [OPERAND]
    // The instruction block starts with the result flag.
    let resonate_opcode_idx = bytes
        .iter()
        .position(|&b| b == 0x33)
        .expect("OP_RESONATE not found in emitted bytecode");
    assert_eq!(
        bytes[resonate_opcode_idx + 1],
        1,
        "Direction byte should be 1 for TEAM_B"
    );

    // Verify VM can successfully deserialize the modified bytecode without error
    let _vm = phiflow::phi_ir::vm::PhiVm::from_bytes(&bytes)
        .expect("VM deserialization failed on direction byte");
}

#[test]
fn test_world_class_fundamentals_compiles() {
    let source = include_str!("../examples/world_class_fundamentals.phi");
    let exprs = parse_phi_program(source).expect("parse failed");
    let program = lower_program(&exprs);
    let mut emitter = OpenQasmEmitter::new();
    let qasm = emitter.emit(&program).expect("emit failed");
    ...
    assert!(qasm.contains("ry(1 * pi)")); // TEAM_A ry(1*pi) decomposed

    // Verify The_Duck (k=2, contradiction) — Heron-native decomposition
    assert!(qasm.contains("ry(1 * pi)")); // TEAM_A ry(1*pi) decomposed on q[3]
    assert!(qasm.contains("ry(pi - (1 * pi))")); // TEAM_B inversion decomposed
